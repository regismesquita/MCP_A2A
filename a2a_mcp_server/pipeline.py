...
class PipelineDefinition:
    ...
    def build_execution_order(self) -> List[str]:
        ...
        def visit(node_id):
            ...
            if node_id in temp:
                raise ValueError(
                    f"Cycle detected in pipeline involving node '{node_id}'"
                )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Pipeline: {self.definition['name']} - "
            f"{len(self.definition['nodes'])} nodes"
        )
...
class PipelineState:
    ...
    def update_progress(self) -> None:
        ...
        if self.status in [
            ...
            self.progress = (
                1.0 if self.status == PipelineStatus.COMPLETED else self.progress
            )
            return
...
class PipelineExecutionEngine:
    ...
    async def _execute_pipeline_nodes(
        self, pipeline_id: str, initial_input: Union[str, Dict[str, Any]], ctx
    ) -> None:
        """
        ...
        for i, node_id in enumerate(pipeline_state.execution_order):
            ...
            try:
                ...
                result = await self._execute_node(
                    pipeline_state, node_id, node_inputs, ctx
                )
        ...
        for node_id in pipeline_state.definition.definition["final_outputs"]:
            if node_id in inputs:
                final_outputs.append({"node_id": node_id, "artifacts": inputs[node_id]})
    ...
    def _map_inputs(
        self, pipeline_state: PipelineState, node_id: str, inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for input_name, mapping in input_mappings.items():
            ...
            artifact = pipeline_state.get_artifact_from_node(
                source_node, source_artifact
            )
            if artifact:
                mapped_inputs[input_name] = artifact
            else:
                logger.warning(
                    f"Artifact {source_artifact} not found in node {source_node}"
                )

        return mapped_inputs

    async def _execute_node(
        self, pipeline_state: PipelineState, node_id: str, inputs: Dict[str, Any], ctx
        ...
        try:
            ...
            if result.get("status") == "error":
                ...
                logger.error(
                    f"Error calling agent {agent_name} for node {node_id}: {error_msg}"
                )
                node_state.status = NodeStatus.FAILED
