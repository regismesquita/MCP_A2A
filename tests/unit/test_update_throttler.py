...
def test_batch_size_triggering(throttler):
    ...
    batch_size = 3
    custom_throttler = UpdateThrottler(min_interval=1.0, batch_size=batch_size, max_queue_size=10)
    key = "test-key"
    ...
    should_send, batch = custom_throttler.add_update(key, {"status": "working", "progress": 0.1})
    assert should_send is True
...
def test_queue_size_management():
    ...
    max_size = 5
    throttler = UpdateThrottler(min_interval=1.0, batch_size=10, max_queue_size=max_size)
    key = "test-key"
...
def test_clear_all():
    ...
    assert sum(len(throttler.get_pending_updates(key)) for key in ["key1", "key2", "key3"]) > 0
